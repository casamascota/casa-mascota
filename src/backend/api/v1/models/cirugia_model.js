...
exports.getCirugiaByDoctorIdsql = (doctorId, callback) => {
    db.all('SELECT * FROM Cirugia WHERE Doctor_id_doctor = ?', [doctorId], (err, rows) => {
      if (err) {
        console.error(err.message);
        callback(err);
      } else {
...
exports.getCirugiasByEnfermeroIdsql = (enfermeroId, callback) => {
    db.all('SELECT * FROM Cirugia WHERE Enfermero_id_enfermero = ?', [enfermeroId], (err, rows) => {
      if (err) {
        console.error(err.message);
        callback(err);
      } else {
...
exports.getCirugiaByDiagnosticoIdsql = (DiagnosticoId, callback) => {
    db.all('SELECT * FROM Cirugia WHERE Cirugia_Diagnostico = ?', [DiagnosticoId], (err, rows) => {
      if (err) {
        console.error(err.message);
        callback(err);
      } else {
...
exports.createCirugiasql = (revision, callback) => {
  const { id, fecha, cirugiahecha, Doctor_id_doctor, Enfermero_id_enfermero, Diagnostico_id_diagnostico } = revision;
  db.run(
    ...
    [id, fecha, cirugiahecha, Doctor_id_doctor, Enfermero_id_enfermero, Diagnostico_id_diagnostico],
    (err) => {
      if (err) {
        console.error(err.message);
        callback(err);
...
exports.updateCirugiasql = (id, cirugia, callback) => {
  const { fecha, cirugiahecha, Doctor_id_doctor, Enfermero_id_enfermero, Diagnostico_id_diagnostico } = cirugia;
  db.run(
    ...
    [fecha, cirugiahecha, Doctor_id_doctor, Enfermero_id_enfermero, Diagnostico_id_diagnostico, id],
    (err) => {
      if (err) {
        console.error(err.message);
        callback(err);
